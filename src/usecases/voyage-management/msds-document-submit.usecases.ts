...
import { MSDSDocumentUploadSubmitRequestDto, MSDSDocumentUploadSubmitResponseDto } from "@src/controllers/voyage-management/dto/msds-document-upload-submit.dto";
...
export class MsdsDocumentSubmitUseCases {
  constructor(
  ...
  async execute(
    ...
    XRequestId: string,
    TimeZone: string,
    payload: MSDSDocumentUploadSubmitRequestDto
  ): Promise<any> {
    this.logger.log(`MSDS Document Submit Request payload ${JSON.stringify(payload)}`);
    this.logger.log(
    ...
    return new MSDSDocumentUploadSubmitResponseDto(ServiceProviderData?.data?.code, ServiceProviderData?.data?.messages, ServiceProviderData?.data?.type);
  }
}
